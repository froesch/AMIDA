...
import functionalunit.cache.ObjectCache;
...
//	private TraceManager traceManager;
	...
	public ObjectHeap(Memory mem, String configFile, TraceManager traceManager, boolean synthesis) {
		super(HeapOpcodes.class, configFile, traceManager);
		this.mem = mem;
		objectCache = new Cache(mem, configFile, 99, synthesis, traceManager);
		htCache = objectCache.getHTCache();
...
//		this.traceManager = traceManager;
	...
	public void initHeap(AXTLoader axtLoader){
		...
		if(activeTrace) trace = new ObjectHeapTrace("HeapTrace_" + axtLoader.nameByPath() + ".BIN");
	}
...
//			System.out.println("---------->Out high (previous tick): " + output[RESULT_HIGH]);			
			...
			case READ:
				...
				case 0:
					...
					if(delay == 0){
						output[RESULT_LOW] = objectCache.getData();
			...
			case READ_64:
				...
				case 1:
					...
					case 0:
						...
						if(delay == 0){
							output[RESULT_HIGH] = objectCache.getData();
							setOutputValid(RESULT_LOW);
...
//						System.out.println("CCCTTTII " + output[RESULT_LOW]);
						setOutputValid(RESULT_LOW);
...
//					System.out.println("DIMSIZE: "  + input[OPERAND_B_LOW]);
			...
			case PHY_READ:
				...
				output[RESULT_LOW] = mem.read(address);
				setOutputValid(RESULT_LOW);
			...
			case PHY_WRITE:
				this.address = input[OPERAND_A_LOW];
				mem.write(address, input[OPERAND_C_LOW]);
				setResultAck(true);
	...
	public void registerHandle(int index, int mid, int flags, int classTableIndex,
...
//		mem.registerHandle(index, mid, flags, classTableIndex, refObjectSize, absoluteReference);
		htCache.writeData(index, absoluteReference, refObjectSize, classTableIndex, flags, mid);
		this.handles++;
	}
...
//		System.out.println("alloc multi dim: RegisterHandle with Addr="+free+" and Size="+dimsizes[dim]+" in Handle "+handles);
		this.registerHandle(handles, 0, flags, cti, dimsizes[dim], free);		
